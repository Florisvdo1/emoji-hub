...
const emojiData = {
  smileys: [
    ...
    '😶','😏','😒','🙄','😬','🤥','😌','😔','😪','🤤',
    '😴','😷','🤒','🤕','🤢','🤮','🤧','😵','🤯','🤠',
    // ... (add more to reach over 30 emojis)
  ],
  animals: [
    '🐶','🐱','🐭','🐹','🐰','🦊','🐻','🐼','🐨','🐯',
    '🦁','🐮','🐷','🐸','🐵','🙈','🙉','🙊','🐒','🐔',
...
// Adjust the emoji deck height dynamically
function adjustLayout() {
  ...
  const totalUsedHeight = header.offsetHeight + huiswerkContainer.offsetHeight + planner.offsetHeight;
  const windowHeight = window.innerHeight;

  const emojiDeckHeight = windowHeight - totalUsedHeight;

  if (emojiDeckHeight > 0) {
    emojiDeck.style.setProperty('--emoji-deck-height', `${emojiDeckHeight}px`);
  } else {
    emojiDeck.style.setProperty('--emoji-deck-height', `100px`); // Minimum height
  }
}
